...
export const WARM_ROOM_CONNECTED_PANE_LIMIT_STORAGE_KEY = "space.warmRoom.connectedPaneLimit.v1";
export const DEFAULT_WARM_ROOM_CONNECTED_PANE_LIMIT = 32;
export const MIN_WARM_ROOM_CONNECTED_PANE_LIMIT = 6;
export const MAX_WARM_ROOM_CONNECTED_PANE_LIMIT = 32;
...
export function writeStoredWarmRoomConnectedPaneLimit(value: unknown): number {
  ...
  try {
    getSpaceRuntime().platform.localStorage.setItem(WARM_ROOM_CONNECTED_PANE_LIMIT_STORAGE_KEY, String(normalized));
  } catch {
    // The effective in-memory setting still applies when browser storage is unavailable.
  }
  return normalized;
